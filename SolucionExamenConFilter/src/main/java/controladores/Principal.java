	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (valororden != null) {
			...
			case "Detalles":
				 ...
				 break;
			case "Terminar":
				acciones.accionTerminar(request.getSession());
				break;
			case "MasSaldo":
				users = request.getParameterValues("users");
                if (users != null){
                    acciones.accionMasSaldo(users);
                }
                break;
            case "Bloquear":
            	users = request.getParameterValues("users");
            	if ( users != null){
                acciones.accionBloquear(users);
                } else {
                acciones.accionBloquear(new String[0]);    
                }
                break;
		...
		if (valororden == null ||  valororden.equals("Borrar") ||
		    valororden.equals("Bloquear") || valororden.equals("MasSaldo")  ){
